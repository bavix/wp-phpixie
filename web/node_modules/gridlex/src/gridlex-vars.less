...
.makeGridByNumber(@grid){
  ...
  [class*="@{grid}-11"] > .@{gl-colName}, [class*="@{grid}-10"] > [class*="@{gl-colName}-"]{ flex-basis: @glp_col-11-12; max-width: @glp_col-11-12; }
  [class*="@{grid}-12"] > .@{gl-colName}, [class*="@{grid}-11"] > [class*="@{gl-colName}-"]{ flex-basis: @glp_col-1; max-width: @glp_col-1; }
